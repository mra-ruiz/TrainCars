...
void StudentTests() {
  ...
  //Here we actually remove the last car
  TrainCar* popback_test = PopBack(simple);
  SanityCheck(simple);
  std::cout << "\nAfter PopBack:" <<std::endl;
  PrintTrain(simple);
  std::cout << "Car Popped back:" << popback_test <<std::endl;

  std::cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~TESTING PUSHFRONT~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
  // helper routine sanity check & print the results
  SanityCheck(simple);
  std::cout << "Before PushFront:" << std::endl;
  PrintTrain(simple);

  //Here we actually add the car to the front
  PushFront(simple, popback_test);
  SanityCheck(simple);
  std::cout << "\nAfter PushFront (pushing car we removed in PopBack):" <<std::endl;
  PrintTrain(simple);
  std::cout << "Car Pushed front:" << popback_test <<std::endl;
  ...
  //make a pointer to point to car number 350
  TrainCar* car_350 = simple;
  for (int i = 0; i < 3; i++) {
      car_350 = car_350->next;
  }

  //Here we actually pop the car
  TrainCar* pop_test = Pop(car_350);
  SanityCheck(simple);
  std::cout << "\nAfter Pop (popped car 350):" <<std::endl;
  PrintTrain(simple);
  std::cout << "Car Popped:" << pop_test <<std::endl;

  std::cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~TESTING INSERTBEFORE~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
  // helper routine sanity check & print the results
  SanityCheck(simple);
  std::cout << "Before InsertBefore:" << std::endl;
  PrintTrain(simple);

  //make a pointer to point to first engine
  TrainCar* first_engine = simple;
  while(!first_engine->isEngine()) {
      first_engine = first_engine -> next;
  }
  //Here we actually insert the car we just popped before the engine
  InsertBefore(first_engine, pop_test);
  SanityCheck(simple);
  std::cout << "\nAfter InsertBefore (pushing car we popped):" <<std::endl;
  PrintTrain(simple);
  std::cout << "Car Inserted Before:" << pop_test <<std::endl;

  std::cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~TESTING INSERTAFTER~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
  // helper routine sanity check & print the results
  SanityCheck(simple);
  std::cout << "Before InsertAfter:" << std::endl;
  PrintTrain(simple);

  //make a pointer to point to first engine
  TrainCar* firstengine = simple;
  while(!firstengine->isEngine()) {
      firstengine = firstengine -> next;
  }
  //make a pointer to car 350
  TrainCar* car350 = simple;
  car350 = car350->next;
  TrainCar* popped = Pop(car350);

  //Here we actually insert the car we just popped after the engine
  InsertAfter(firstengine, popped);
  SanityCheck(simple);
  std::cout << "\nAfter InsertAfter (car 350):" <<std::endl;
  PrintTrain(simple);
  std::cout << "Car Inserted After:" << popped <<std::endl;
